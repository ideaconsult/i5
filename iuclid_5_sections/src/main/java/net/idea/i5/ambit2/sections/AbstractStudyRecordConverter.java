...
public abstract class AbstractStudyRecordConverter<T, PROTOCOLPARAMS extends IParams, CONDITIONS extends IParams>
	...
	protected boolean hasDataTransferCriteriaFulfilled(T unmarshalled) {
		if (!hasScientificPart(unmarshalled))
			return false;
		if (isDataWaiving(unmarshalled))
			return false;
	...
	public ambit2.base.interfaces.IStructureRecord transform2record(T unmarshalled, SubstanceRecord record)
			throws AmbitException {
		if (hasDataTransferCriteriaFulfilled(unmarshalled))
			return null;
		else
			return record;
	};
	...
	protected Params parseReliability(ProtocolApplication papp, String valueID, boolean isRobustStudy,
			boolean isUsedforClassification, boolean isUsedforMSDS, String purposeFlagCode, String studyResultTypeID,
	...
	protected PROTOCOLPARAMS createProtocolParameters() {
		if (endpointCategory == null)
			...
			return (PROTOCOLPARAMS) endpointCategory.createProtocolParameters();
	}
