...
public class StudyRecordConverter extends ECOTOXStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20130101.studyrecord.EC_PLANTTOX_SECTION.EndpointStudyRecord>{
	...
	public IStructureRecord transform2record(EndpointStudyRecord unmarshalled, SubstanceRecord record) throws AmbitException {
		...
		if (sciPart.getECPLANTTOX().getEXPDURATION()!=null) {
			Params p = new Params();
			try {p.setLoValue( sciPart.getECPLANTTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getVALUE().getValue());}
			catch (Exception x ){p.setLoValue(null);}
			try {p.setUnits(sciPart.getECPLANTTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getUNITValue());} catch (Exception x){}
		...
		try {
			String medium = sciPart.getECPLANTTOX().getSUBSTRATETYPE().getSet().getPHRASEOTHERLISTPOP().getLISTPOPValue();
			if (medium.startsWith("other:")) medium = sciPart.getECPLANTTOX().getSUBSTRATETYPE().getSet().getPHRASEOTHERLISTPOP().getLISTPOPTXT().getValue();
