...
public class StudyRecordConverter
	extends
	PChemStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20130101.studyrecord.PC_PARTITION_SECTION.EndpointStudyRecord, IParams, IParams> {
	...
	try {
	    return unmarshalled.getScientificPart().getPCPARTITION().getTESTMATINDICATOR().getSet().getLISTBELOWSEL()
		    .getLISTBELOWSEL();
	} catch (Exception x) {
	    return null;
	}
	...
	if (sciPart.getPCPARTITION().getMETHODNOGUIDELINE() != null)
	    try {
		papp.getProtocol().addGuideline(
			sciPart.getPCPARTITION().getMETHODNOGUIDELINE().getSet().getTEXTAREABELOW().getTEXTAREABELOW()
				.getValue());
	    } catch (Exception x) {
	    }
	/*
	...
	if (sciPart.getPCPARTITION().getMETHODTYPE() != null) {
	    papp.getParameters().put(
		    ...
		    getValue(sciPart.getPCPARTITION().getMETHODTYPE().getSet().getPHRASEOTHERLISTPOPFIX()
			    .getLISTPOPFIXValue(), sciPart.getPCPARTITION().getMETHODTYPE().getSet()
	...
	for (eu.europa.echa.schemas.iuclid5._20130101.studyrecord.PC_PARTITION_SECTION.EndpointStudyRecord.ScientificPart.PCPARTITION.PARTCOEFF.Set set : sciPart
		...
		if (set.getPRECISIONPHLOQUALIFIER().getPHLOVALUE() != null) {
		    phvalue.setLoQualifier(getQualifier(set.getPRECISIONPHLOQUALIFIER().getPHLOQUALIFIERValue()));
		...
		if (set.getPRECISIONPHLOQUALIFIER().getPHUPVALUE() != null) {
		    phvalue.setUpQualifier(getQualifier(set.getPRECISIONPHLOQUALIFIER().getPHLOQUALIFIERValue()));
