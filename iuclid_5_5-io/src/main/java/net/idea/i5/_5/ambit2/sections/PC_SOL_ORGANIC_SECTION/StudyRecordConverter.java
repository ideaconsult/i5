...
public class StudyRecordConverter
	extends
	PChemStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20130101.studyrecord.PC_SOL_ORGANIC_SECTION.EndpointStudyRecord, IParams, IParams> {
	...
	try {
	    return unmarshalled.getScientificPart().getPCSOLORGANIC().getTESTMATINDICATOR().getSet().getLISTBELOWSEL()
		    .getLISTBELOWSEL();
	} catch (Exception x) {
	    return null;
	}
	...
	if (super.transform2record(unmarshalled, record) == null)
	    return null;
	eu.europa.echa.schemas.iuclid5._20130101.studyrecord.PC_SOL_ORGANIC_SECTION.EndpointStudyRecord.ScientificPart sciPart = unmarshalled
		.getScientificPart();
	...
	if (sciPart.getPCSOLORGANIC().getGUIDELINE() != null)
	    for (ScientificPart.PCSOLORGANIC.GUIDELINE.Set set : sciPart.getPCSOLORGANIC().getGUIDELINE().getSet()) {
	...
	if (sciPart.getPCSOLORGANIC().getMETHODNOGUIDELINE() != null)
	    try {
		papp.getProtocol().addGuideline(
			sciPart.getPCSOLORGANIC().getMETHODNOGUIDELINE().getSet().getTEXTAREABELOW().getTEXTAREABELOW()
				.getValue());
	    } catch (Exception x) {
	    }
	/*
	...
	for (eu.europa.echa.schemas.iuclid5._20130101.studyrecord.PC_SOL_ORGANIC_SECTION.EndpointStudyRecord.ScientificPart.PCSOLORGANIC.FATSOL.Set set : sciPart
		...
		effect.getConditions().put(
			...
			getValue(set.getPHRASEOTHERORGANICMEDIUM().getORGANICMEDIUMValue(), set
				.getPHRASEOTHERORGANICMEDIUM().getORGANICMEDIUMTXT()));
	    } catch (Exception x) {
