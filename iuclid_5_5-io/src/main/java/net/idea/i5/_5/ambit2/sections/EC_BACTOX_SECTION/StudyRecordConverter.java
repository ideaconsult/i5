...
public class StudyRecordConverter extends ECOTOXStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20130101.studyrecord.EC_BACTOX_SECTION.EndpointStudyRecord>{
	...
	protected String getTestMaterialIdentity(EndpointStudyRecord unmarshalled) {
		try {
			return unmarshalled.getScientificPart().getECBACTOX().getTESTMATINDICATOR().getSet().getLISTBELOWSEL().getLISTBELOWSEL();
		} catch (Exception x) {
			return null;	
		}
	}
	...
	public IStructureRecord transform2record(EndpointStudyRecord unmarshalled, SubstanceRecord record) throws AmbitException {
		...
		if (sciPart.getECBACTOX().getGUIDELINE()!=null)
			for (eu.europa.echa.schemas.iuclid5._20130101.studyrecord.EC_BACTOX_SECTION.EndpointStudyRecord.ScientificPart.ECBACTOX.GUIDELINE.Set set : sciPart.getECBACTOX().getGUIDELINE().getSet()) try {
		...
		if (sciPart.getECBACTOX().getEXPDURATION()!=null) {
			Params p = new Params();
			try {p.put(I5CONSTANTS.loValue, sciPart.getECBACTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getVALUE().getValue());}
			catch (Exception x) {p.put(I5CONSTANTS.loValue, null);}
			try {p.put(I5CONSTANTS.unit,sciPart.getECBACTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getUNITValue());} catch (Exception x){}
		...
		if (sciPart.getECBACTOX().getWATERTYPE()!=null) {
				papp.getParameters().put(I5CONSTANTS.cTestMedium,sciPart.getECBACTOX().getWATERTYPE().getSet().getLISTRIGHTPOP().getLISTRIGHTPOPValue());
		} else 
