package net.idea.i5._5.ambit2.sections.POUR_DENSITY_SECTION;
...
import eu.europa.echa.schemas.iuclid5._20130101.studyrecord.POUR_DENSITY_SECTION.EndpointStudyRecord;
import eu.europa.echa.schemas.iuclid5._20130101.studyrecord.POUR_DENSITY_SECTION.EndpointStudyRecord.ScientificPart;
import eu.europa.echa.schemas.iuclid5._20130101.studyrecord.POUR_DENSITY_SECTION.EndpointStudyRecord.ScientificPart.POURDENSITY.REFERENCE.Set;


public class StudyRecordConverter
		extends
		PChemStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20130101.studyrecord.POUR_DENSITY_SECTION.EndpointStudyRecord> {
	...
	protected String getTestMaterialIdentity(EndpointStudyRecord unmarshalled) {
		try {
			return unmarshalled.getScientificPart().getPOURDENSITY().getIDENTITYOFTESTMATERIAL().getSet().getLISTBELOWSEL().getLISTBELOWSELValue();
		} catch (Exception x) {
			return null;	
		}
	}	
	...
	public IStructureRecord transform2record(EndpointStudyRecord unmarshalled,
		...
		if (sciPart.getPOURDENSITY().getMETHODNOGUIDELINE() != null)
			try {
				papp.getProtocol().addGuideline(
						sciPart.getPOURDENSITY().getMETHODNOGUIDELINE()
		...
		papp.getParameters().put(I5CONSTANTS.cTypeMethod,
					sciPart.getPOURDENSITY().getMETHOD()==null?null:
					sciPart.getPOURDENSITY().getMETHOD().getSet().getPHRASEOTHERLISTSELFIX().getLISTSELFIXValue());
		
		try {
			papp.getParameters().put(I5CONSTANTS.pMETHODDETAILS,
				sciPart.getPOURDENSITY().getMETHODSDETAILSDATAEVAL().getSet().getTEXTAREABELOW().getTEXTAREABELOW().getValue());
		} catch (Exception x) {
			papp.getParameters().put(I5CONSTANTS.pMETHODDETAILS,null);
		}
		try {
			papp.getParameters().put(I5CONSTANTS.pTESTMAT_FORM,
				sciPart.getPOURDENSITY().getTESTMATFORM()==null?null:
				sciPart.getPOURDENSITY().getTESTMATFORM().getSet().getPHRASEOTHERTESTMATFORMNM25().getTESTMATFORMNM25Value());
		} catch (Exception x) {
			papp.getParameters().put(I5CONSTANTS.pTESTMAT_FORM,null);
		}
		try {
			papp.getParameters().put(I5CONSTANTS.pSAMPLING,sciPart.getPOURDENSITY().getSAMPLING().getSet().getFREETEXTBELOW().getFREETEXTBELOW().getValue());
		} catch (Exception x) {
			papp.getParameters().put(I5CONSTANTS.pSAMPLING,null);
		}
		try {
			papp.getParameters().put(I5CONSTANTS.pDATA_GATHERING_INSTRUMENTS,sciPart.getPOURDENSITY().getDATAGATHERINGINSTRUMENTS().getSet().getTEXTAREABELOW().getTEXTAREABELOW().getValue());
		} catch (Exception x) {
			papp.getParameters().put(I5CONSTANTS.pDATA_GATHERING_INSTRUMENTS,null);
		}
				
		
		if (sciPart.getPOURDENSITY().getPOURDENSITYTABLE()!=null) {
			parseElement(sciPart.getPOURDENSITY().getPOURDENSITYTABLE(),papp);
		}
		try {
			papp.setInterpretationResult(sciPart.getPOURDENSITY().getAPPLCL().getSet().getTEXTAREABELOW().getTEXTAREABELOW().getValue());
		} catch (Exception x) {
