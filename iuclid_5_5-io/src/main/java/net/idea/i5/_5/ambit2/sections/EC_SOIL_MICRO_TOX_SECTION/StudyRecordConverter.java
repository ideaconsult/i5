...
public class StudyRecordConverter extends ECOTOXStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20130101.studyrecord.EC_SOIL_MICRO_TOX_SECTION.EndpointStudyRecord>{
	...
	public IStructureRecord transform2record(EndpointStudyRecord unmarshalled, SubstanceRecord record) throws AmbitException {
		...
		if (sciPart.getECSOILMICROTOX().getGUIDELINE()!=null)
			for (eu.europa.echa.schemas.iuclid5._20130101.studyrecord.EC_SOIL_MICRO_TOX_SECTION.EndpointStudyRecord.ScientificPart.ECSOILMICROTOX.GUIDELINE.Set set : sciPart.getECSOILMICROTOX().getGUIDELINE().getSet()) try {
		...
		if (sciPart.getECSOILMICROTOX().getEXPDURATION()!=null) {
			Params p = new Params();
			try {p.put(I5CONSTANTS.loValue, sciPart.getECSOILMICROTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getVALUE().getValue());} 
			catch (Exception x) {p.put(I5CONSTANTS.loValue,null); }
			try {p.put(I5CONSTANTS.unit,sciPart.getECSOILMICROTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getUNITValue());} catch (Exception x) {}
