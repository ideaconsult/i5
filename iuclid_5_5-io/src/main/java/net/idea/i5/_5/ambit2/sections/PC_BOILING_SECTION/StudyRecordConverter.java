...
public class StudyRecordConverter
	...
	try {
	    return unmarshalled.getScientificPart().getPCBOILING().getTESTMATINDICATOR().getSet().getLISTBELOWSEL()
		    .getLISTBELOWSEL();
	} catch (Exception x) {
	    return null;
	}
	...
	if (sciPart.getPCBOILING().getGUIDELINE() != null)
	    for (ScientificPart.PCBOILING.GUIDELINE.Set set : sciPart.getPCBOILING().getGUIDELINE().getSet())
		try {
	...
	if (sciPart.getPCBOILING().getMETHODNOGUIDELINE() != null)
	    try {
		papp.getProtocol().addGuideline(
			sciPart.getPCBOILING().getMETHODNOGUIDELINE().getSet().getTEXTAREABELOW().getTEXTAREABELOW()
				.getValue());
	    } catch (Exception x) {
	    }
	...
	if (sciPart.getPCBOILING().getBOILINGPT() != null && sciPart.getPCBOILING().getBOILINGPT().getSet() != null)
		...
		try {
		    effect.getConditions().put(I5CONSTANTS.rDECOMPOSITION,
			    set.getPHRASEOTHERDECOMPINDICATOR().getDECOMPINDICATORValue());
		} catch (Exception x) {
		...
		if (set.getPRECISIONPRESSURELOQUALIFIER() != null) {
			...
			vpvalue.setLoQualifier((set.getPRECISIONPRESSURELOQUALIFIER().getPRESSURELOQUALIFIERValue() == null) ? null
				: set.getPRECISIONPRESSURELOQUALIFIER().getPRESSURELOQUALIFIERValue());
			vpvalue.setLoValue(getNumber(set.getPRECISIONPRESSURELOQUALIFIER().getPRESSURELOVALUE()
				.getValue()));
		    } else
			vpvalue.setLoValue(null);

		    if (set.getPRECISIONPRESSURELOQUALIFIER().getPRESSUREUPVALUE() != null) {
			vpvalue.setUpQualifier((set.getPRECISIONPRESSURELOQUALIFIER().getPRESSUREUPQUALIFIER() == null) ? null
				: set.getPRECISIONPRESSURELOQUALIFIER().getPRESSUREUPQUALIFIERValue());
			vpvalue.setUpValue(getNumber(set.getPRECISIONPRESSURELOQUALIFIER().getPRESSUREUPVALUE()
				.getValue()));
		    } else
			vpvalue.setUpValue(null);
		    vpvalue.setUnits(Phrases.phrasegroup_P02.get(set.getPRECISIONPRESSURELOQUALIFIER()
			    .getPRESSUREUNIT()));
