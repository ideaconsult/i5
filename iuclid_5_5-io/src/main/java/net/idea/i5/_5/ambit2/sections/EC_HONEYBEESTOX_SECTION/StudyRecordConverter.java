...
public class StudyRecordConverter extends ECOTOXStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20130101.studyrecord.EC_HONEYBEESTOX_SECTION.EndpointStudyRecord>{
	...
	public IStructureRecord transform2record(EndpointStudyRecord unmarshalled, SubstanceRecord record) throws AmbitException {
		...
		if (sciPart.getECHONEYBEESTOX().getEXPDURATION()!=null) {
			Params p = new Params();
			try {p.put(I5CONSTANTS.loValue, sciPart.getECHONEYBEESTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getVALUE().getValue());}
			catch (Exception x) {p.put(I5CONSTANTS.loValue, null);}
			try {p.put(I5CONSTANTS.unit,sciPart.getECHONEYBEESTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getUNITValue());} catch (Exception x){}
		...
		if (sciPart.getECHONEYBEESTOX().getORGANISM()!=null) {
			papp.getParameters().put(I5CONSTANTS.cTestOrganism,sciPart.getECHONEYBEESTOX().getORGANISM().getSet().getPHRASEOTHERLISTPOP().getLISTPOPValue());
		} else	
