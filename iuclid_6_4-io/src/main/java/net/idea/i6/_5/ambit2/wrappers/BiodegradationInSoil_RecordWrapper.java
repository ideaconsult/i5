...
import eu.europa.echa.iuclid6.namespaces.endpoint_study_record_biodegradationinsoil._5.ENDPOINTSTUDYRECORDBiodegradationInSoil;
import eu.europa.echa.iuclid6.namespaces.endpoint_study_record_biodegradationinsoil._5.ENDPOINTSTUDYRECORDBiodegradationInSoil.MaterialsAndMethods.StudyDesign.SoilProperties.Entry;
...
public class BiodegradationInSoil_RecordWrapper
	...
	public void assignEffectLevels(ProtocolApplication papp, ENDPOINTSTUDYRECORDBiodegradationInSoil studyrecord) {
		if (studyrecord.getMaterialsAndMethods().getStudyDesign().getSoilProperties()!=null)
		...
		if (studyrecord.getResultsAndDiscussion().getDegradation() != null)
			for (eu.europa.echa.iuclid6.namespaces.endpoint_study_record_biodegradationinsoil._5.ENDPOINTSTUDYRECORDBiodegradationInSoil.ResultsAndDiscussion.Degradation.Entry e : studyrecord.getResultsAndDiscussion().getDegradation().getEntry()) {
		...
		if (studyrecord.getResultsAndDiscussion().getHalfLifeOfParentCompound() != null)
			for (eu.europa.echa.iuclid6.namespaces.endpoint_study_record_biodegradationinsoil._5.ENDPOINTSTUDYRECORDBiodegradationInSoil.ResultsAndDiscussion.HalfLifeOfParentCompound.Entry e : studyrecord
		 ...
		 * q2effectrecord(e.getDegr(), effect); papp.addEffect(effect);
		 * 
		 * effect.getConditions().put(I5CONSTANTS.cSoilNo,
		 * p2Value(e.getSoilNo()));
		 * effect.getConditions().put(I5CONSTANTS.cTestType,
		 * p2Value(studyrecord.getMaterialsAndMethods().getTestType()));
		 * 
