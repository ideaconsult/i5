...
import eu.europa.echa.iuclid6.namespaces.endpoint_study_record_surfacechemistry._5.ENDPOINTSTUDYRECORDSurfaceChemistry;
import eu.europa.echa.iuclid6.namespaces.endpoint_study_record_surfacechemistry._5.ENDPOINTSTUDYRECORDSurfaceChemistry.ResultsAndDiscussion.CoatingSet.AtomicComposition.Entry;
	...
	public void assignEffectLevels(ProtocolApplication papp, ENDPOINTSTUDYRECORDSurfaceChemistry studyrecord) {
		...
		for (Entry entry : studyrecord.getResultsAndDiscussion().getCoating().getAtomicComposition().getEntry()) {
			effect = addEffectRecord_meanstdev(papp, I5CONSTANTS.eFRACTION, entry.getFraction(),
					entry.getStDev() == null ? null : entry.getStDev().getValue().doubleValue(),null);
		...
		papp.addEffect(effect);
		for (eu.europa.echa.iuclid6.namespaces.endpoint_study_record_surfacechemistry._5.ENDPOINTSTUDYRECORDSurfaceChemistry.ResultsAndDiscussion.Functionalisation.FunctionalGroup.Entry entry : studyrecord.getResultsAndDiscussion().getFunctionalisation().getFunctionalGroup().getEntry()) {
			effect = addEffectRecord_meanstdev(papp, I5CONSTANTS.eFRACTION, entry.getFraction(),
					entry.getStDev() == null ? null : entry.getStDev().getValue().doubleValue(),null);
