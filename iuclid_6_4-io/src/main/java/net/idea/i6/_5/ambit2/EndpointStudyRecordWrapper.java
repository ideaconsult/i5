...
public class EndpointStudyRecordWrapper<STUDYRECORD> extends AbstractDocWrapper {
  ...
  public EndpointStudyRecordWrapper(Document doc) throws Exception {
    super(doc);
    endpointCategory = I6_ROOT_OBJECTS.valueOf(
        String.format("%s_%s", getPlatformMetadataValue("documentType"), getPlatformMetadataValue("documentSubType")));
        ...
        if (m.getName().startsWith("get"))
          try {
            ...
            logger.log(Level.WARNING,
                String.format("%s\t%s", value == null ? null : value.getClass().getName(), m.getName()), x);
          }
  ...
  protected boolean isStudyResultAccepted(String studyResultTypeID, QASettings qaSettings) throws QACriteriaException {
