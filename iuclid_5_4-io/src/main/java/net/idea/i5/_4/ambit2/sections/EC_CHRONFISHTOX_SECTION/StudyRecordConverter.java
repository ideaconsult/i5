package net.idea.i5._4.ambit2.sections.EC_CHRONFISHTOX_SECTION;
...
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONFISHTOX_SECTION.DocumentTypeType;
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONFISHTOX_SECTION.EndpointStudyRecord;
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONFISHTOX_SECTION.EndpointStudyRecord.ScientificPart.ECCHRONFISHTOX.REFERENCE.Set;

public class StudyRecordConverter
	extends
	ECOTOXStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONFISHTOX_SECTION.EndpointStudyRecord> {
	...
	try {
	    return unmarshalled.getScientificPart().getECCHRONFISHTOX().getTESTMATINDICATOR().getSet()
	...
	if (super.transform2record(unmarshalled, record) == null)
	    return null;
	eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONFISHTOX_SECTION.EndpointStudyRecord.ScientificPart sciPart = unmarshalled
		.getScientificPart();
	if (sciPart.getECCHRONFISHTOX() == null)
	    return null;
	...
	if (sciPart.getECCHRONFISHTOX().getGUIDELINE() != null)
	    for (eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONFISHTOX_SECTION.EndpointStudyRecord.ScientificPart.ECCHRONFISHTOX.GUIDELINE.Set set : sciPart
		    .getECCHRONFISHTOX().getGUIDELINE().getSet()) {
		papp.getProtocol().addGuideline(
			getGuideline(set.getPHRASEOTHERGUIDELINE().getGUIDELINEValue(), set.getPHRASEOTHERGUIDELINE()
				.getGUIDELINETXT()));

	    }
	if (sciPart.getECCHRONFISHTOX().getMETHODNOGUIDELINE() != null)
	    try {
		papp.getProtocol().addGuideline(
			sciPart.getECCHRONFISHTOX().getMETHODNOGUIDELINE().getSet().getTEXTAREABELOW()
	...
	if (sciPart.getECCHRONFISHTOX().getEXPDURATION() != null) {
	    Value p = new Value();
	    try {
		p.setLoValue(sciPart.getECCHRONFISHTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getVALUE()
			.getValue());
	    } catch (Exception x) {
		p.setLoValue(null);
	    }
	    try {
		p.setUnits(sciPart.getECCHRONFISHTOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getUNITValue());
	    } catch (Exception x) {
	    }
	...
	if (sciPart.getECCHRONFISHTOX().getWATERTYPE() != null) {
	    papp.getParameters().put(I5CONSTANTS.cTestMedium,
		    sciPart.getECCHRONFISHTOX().getWATERTYPE().getSet().getLISTRIGHTPOP().getLISTRIGHTPOPValue());
	} else
	...
	try {
	    papp.getParameters().put(
		    ...
		    getValue(sciPart.getECCHRONFISHTOX().getORGANISM().getSet().getPHRASEOTHERLISTPOP()
			    .getLISTPOPValue(), sciPart.getECCHRONFISHTOX().getORGANISM().getSet()
	...
	// ENDPOINT
	if (sciPart.getECCHRONFISHTOX().getEFFCONC() != null
		&& sciPart.getECCHRONFISHTOX().getEFFCONC().getSet() != null)
	    for (eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONFISHTOX_SECTION.EndpointStudyRecord.ScientificPart.ECCHRONFISHTOX.EFFCONC.Set set : sciPart
		    .getECCHRONFISHTOX().getEFFCONC().getSet()) {
		...
		if (set.getPRECISIONCONCLOQUALIFIER() != null) {
			...
			try {
			    effect.setLoValue(Double.parseDouble(set.getPRECISIONCONCLOQUALIFIER().getCONCLOVALUE()
				    .getValue()));
			...
			try {
			    effect.setUpValue(Double.parseDouble(set.getPRECISIONCONCLOQUALIFIER().getCONCUPVALUE()
				    .getValue()));
