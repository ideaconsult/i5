package net.idea.i5._4.ambit2.sections.EN_BIOACCU_TERR_SECTION;
...
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EN_BIOACCU_TERR_SECTION.DocumentTypeType;
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EN_BIOACCU_TERR_SECTION.EndpointStudyRecord;
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EN_BIOACCU_TERR_SECTION.EndpointStudyRecord.ScientificPart;
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EN_BIOACCU_TERR_SECTION.EndpointStudyRecord.ScientificPart.ENBIOACCUTERR.REFERENCE.Set;

public class StudyRecordConverter
		extends
		ENVFATEStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EN_BIOACCU_TERR_SECTION.EndpointStudyRecord> {
	...
	protected String getTestMaterialIdentity(EndpointStudyRecord unmarshalled) {
		try {
			return unmarshalled.getScientificPart().getENBIOACCUTERR()
	...
	protected void parseReference(EndpointStudyRecord unmarshalled,
		...
		if (unmarshalled.getScientificPart().getENBIOACCUTERR().getREFERENCE() != null)
			for (Set set : unmarshalled.getScientificPart().getENBIOACCUTERR()
					.getREFERENCE().getSet()) {
				try {
	...
	public IStructureRecord transform2record(EndpointStudyRecord unmarshalled,
		...
		if (super.transform2record(unmarshalled, record) == null)
			return null;
		eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EN_BIOACCU_TERR_SECTION.EndpointStudyRecord.ScientificPart sciPart = unmarshalled
				.getScientificPart();
		if (sciPart.getENBIOACCUTERR() == null)
			return null;
		...
		if (sciPart.getENBIOACCUTERR().getGUIDELINE() != null)
			for (ScientificPart.ENBIOACCUTERR.GUIDELINE.Set set : sciPart
					.getENBIOACCUTERR().getGUIDELINE().getSet()) {
		...
		if (sciPart.getENBIOACCUTERR().getMETHODNOGUIDELINE() != null)
			try {
				papp.getProtocol().addGuideline(
						sciPart.getENBIOACCUTERR().getMETHODNOGUIDELINE()
		...
		try {
			papp.getParameters().put(
					...
					getValue(sciPart.getENBIOACCUTERR().getORGANISM().getSet()
							.getPHRASEOTHERLISTPOP().getLISTPOPValue(), sciPart
							.getENBIOACCUTERR().getORGANISM().getSet()
		...
		if (sciPart.getENBIOACCUTERR().getBCF() != null) {
			for (eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EN_BIOACCU_TERR_SECTION.EndpointStudyRecord.ScientificPart.ENBIOACCUTERR.BCF.Set set : sciPart
					.getENBIOACCUTERR().getBCF().getSet()) {
				...
				try {
					...
					if (basis.startsWith("other:"))
						basis = set.getPHRASEOTHERBASIS().getBASISTXT()
								.getValue();
				...
				if (set.getPRECISIONBSAFLOQUALIFIER() != null) {
					effect.setUnit(set.getPRECISIONBSAFLOQUALIFIER()
							.getBSAFUNITValue());
					if (set.getPRECISIONBSAFLOQUALIFIER().getBSAFLOVALUE() != null)
						try {
							effect.setLoValue(Double.parseDouble(set
									.getPRECISIONBSAFLOQUALIFIER()
									.getBSAFLOVALUE().getValue()));
							effect.setLoQualifier(set
									.getPRECISIONBSAFLOQUALIFIER()
									.getBSAFLOQUALIFIER());
						} catch (Exception x) {
						}
					if (set.getPRECISIONBSAFLOQUALIFIER().getBSAFUPVALUE() != null)
						try {
							effect.setUpValue(Double.parseDouble(set
									.getPRECISIONBSAFLOQUALIFIER()
									.getBSAFUPVALUE().getValue()));
