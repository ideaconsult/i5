package net.idea.i5._4.ambit2.sections.EC_CHRONDAPHNIATOX_SECTION;
...
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONDAPHNIATOX_SECTION.DocumentTypeType;
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONDAPHNIATOX_SECTION.EndpointStudyRecord;
import eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONDAPHNIATOX_SECTION.EndpointStudyRecord.ScientificPart.ECCHRONDAPHNIATOX.REFERENCE.Set;
...
public class StudyRecordConverter
	extends
	ECOTOXStudyRecordConvertor<eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONDAPHNIATOX_SECTION.EndpointStudyRecord> {
	...
	try {
	    return unmarshalled.getScientificPart().getECCHRONDAPHNIATOX().getTESTMATINDICATOR().getSet()
	...
	eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONDAPHNIATOX_SECTION.EndpointStudyRecord.ScientificPart sciPart = unmarshalled
		.getScientificPart();
	if (sciPart.getECCHRONDAPHNIATOX() == null)
	    return null;
	...
	if (sciPart.getECCHRONDAPHNIATOX().getGUIDELINE() != null)
	    for (eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONDAPHNIATOX_SECTION.EndpointStudyRecord.ScientificPart.ECCHRONDAPHNIATOX.GUIDELINE.Set set : sciPart
		    .getECCHRONDAPHNIATOX().getGUIDELINE().getSet())
		try {
	...
	if (sciPart.getECCHRONDAPHNIATOX().getMETHODNOGUIDELINE() != null)
	    try {
		papp.getProtocol().addGuideline(
			sciPart.getECCHRONDAPHNIATOX().getMETHODNOGUIDELINE().getSet().getTEXTAREABELOW()
	...
	if (sciPart.getECCHRONDAPHNIATOX().getEXPDURATION() != null) {
	    Value p = new Value();
	    try {
		p.setLoValue(sciPart.getECCHRONDAPHNIATOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getVALUE()
			.getValue());
	    } catch (Exception x) {
		p.setLoValue(null);
	    }
	    try {
		p.setUnits(sciPart.getECCHRONDAPHNIATOX().getEXPDURATION().getSet().getVALUEUNITVALUE().getUNITValue());
	    } catch (Exception x) {
	    }
	...
	if (sciPart.getECCHRONDAPHNIATOX().getWATERTYPE() != null) {
	    papp.getParameters().put(I5CONSTANTS.cTestMedium,
		    sciPart.getECCHRONDAPHNIATOX().getWATERTYPE().getSet().getLISTRIGHTPOP().getLISTRIGHTPOPValue());
	} else
	...
	try {
	    papp.getParameters().put(
		    ...
		    getValue(sciPart.getECCHRONDAPHNIATOX().getORGANISM().getSet().getPHRASEOTHERLISTPOP()
			    .getLISTPOPValue(), sciPart.getECCHRONDAPHNIATOX().getORGANISM().getSet()
	...
	// ENDPOINT
	if (sciPart.getECCHRONDAPHNIATOX().getEFFCONC() != null
		&& sciPart.getECCHRONDAPHNIATOX().getEFFCONC().getSet() != null)
	    for (eu.europa.echa.schemas.iuclid5._20120101.studyrecord.EC_CHRONDAPHNIATOX_SECTION.EndpointStudyRecord.ScientificPart.ECCHRONDAPHNIATOX.EFFCONC.Set set : sciPart
		    .getECCHRONDAPHNIATOX().getEFFCONC().getSet()) {
